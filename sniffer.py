...
TIME_INTERVAL = 60    # Intervalo de tempo em segundos para contagem dos pacotes
...
def parse_arp_packet(data):
    arp_header = struct.unpack('2s2s1s1s2s6s4s6s4s', data)
...
def sniff_tcp_packets():
    global icmp_packet_count, arp_packet_count
            ...
            if eth_proto == 0x0800:
                ...
                # Verificar se o pacote é do tipo TCP
                    ...
                    if flags & 0x02 != 0:
                        ...
                        # Lógica de detecção de SYN Flood
                        arp_packet_count += 1
